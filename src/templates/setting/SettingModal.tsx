...
const SettingModal = () => {
  const [
    { visibility, options, activeTable, modifiedTable },
    dispatch,
    payload,
  ] = useTableContext();
  ...
  function saveModifiedOptions() {
    ...
    // TableLocalService.updateActiveOptions(activeTable!, modified);
    // handleClose();
  }
    ...
    >
      ...
      <Modal.Body>
        ...
        <BaseRepeater
          ...
          baseKeys={["id", "key", "text", "rate", "tableId", "optionId"]}
          baseTypes={{
            id: "string",
            text: "string",
            key: "string",
          ...
          onBeforeDelete={(i) => {
            ...
            if (!qty) return true;
              ...
              return window.confirm(
                `${msg} таблицы, вы действительно хотите её удалить?`
              );
            }
          }}
          onChange={(list) => {
          ...
          onRender={(state, i) => <EntityRepeater state={state} i={i} />}
        />
        ...
        <div>
          <div style={{ marginBottom: "15px" }}>
            ...
            <span className="small mb-1">
              Примечание: что бы использовать действия на элементе таблицы
              кликните по строке в любое место
            </span>
          </div>
