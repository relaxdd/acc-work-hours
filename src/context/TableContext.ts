...
type DispatchRewrite<T extends Object, K extends keyof T = keyof T> = {
  key: K;
  value: T[K];
};
type RewriteDispatch2 = <T extends ITableStore, K extends keyof T>(
  key: K,
  value: T[K]
) => DispatchRewrite<T>;
...
export type ChangeDateTime = (type: DTEnum, value: string, id: number) => void;
...
type DispatchTable = (
  | { key: "id"; value: number }
  | { key: "start" | "finish" | "description"; value: string }
  | { key: "entityId"; value: number | null }
  | { key: "isPaid"; value: boolean }
) & { id: number };
...
export type ActionOfTReducer =
  ...
  | {
      ...
      payload: { key: ListOfVisibility; value: boolean };
    };
...
function dispatchWorkHours(
  prev: IWorkTableRow[],
  { id, key, value }: DispatchTable
): IWorkTableRow[] {
  return prev.map((it) => {
