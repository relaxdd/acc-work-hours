...
export const getHoursOrZero = (qty: number) =>
  isNaN(qty) ? 0 : ceilHours(qty);
...
export function getAllIds<T extends { id: string | number }>(
  data: T[]
): string[] {
  return data.map((it) => String(it.id));
}
...
export function formToJson(form: FormData) {
  ...
  // @ts-ignore
  for (const [key, value] of form.entries() as Iterable<[string, string]>) {
    obj[key] = value;
  }
