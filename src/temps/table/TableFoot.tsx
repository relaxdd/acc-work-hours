...
const TableFoot = () => {
  ...
  useEffect(() => {
    ...
    } else if (filteredTable.length !== selectedRows.length) {
      if (checkAllRef?.current) {
  ...
  function calcAmountByEntity() {
    ...
    return filtered.reduce<Record<string, number>>((list, it) => {
      ...
      const key =
        options.listOfTech.find((entity) => {
  ...
  const [totalHours, totalPayment] = useMemo(() => {
    ...
    return filtered.reduce(
      (total, it) => {
        ...
        return [total[0] + hours, total[1] + hours * (entity?.rate || 0)];
      },
  ...
  return (
    <tfoot>
      <tr>
        ...
        {!options.hiddenCols.description && <td>&nbsp;</td>}
      </tr>
      <tr>
        <td colSpan={options.hiddenCols.number ? 3 : 4}>
        ...
        <td
          colSpan={
            4 -
            Number(options.hiddenCols.description) -
            Number(options.hiddenCols.entity)
          }
        >
          {textAmountsByLang}
        </td>
